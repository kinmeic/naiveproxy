...
naive_version=${package_version%-*}
naive_release=${package_version##*-}
if [ "$naive_version" = "$naive_release" ]; then
  naive_release=1
fi
...
make package/naiveproxy/compile V=s \
  ...
  NAIVE_VERSION="$naive_version" \
  NAIVE_RELEASE="$naive_release"

mkdir -p "$PACKAGE_OUTPUT_DIR"
package=$(find bin -type f -name "naiveproxy*.${PACKAGE_FORMAT}" -print -quit)
test -n "$package"
cp "$package" "$PACKAGE_OUTPUT_DIR/naiveproxy-$package_version-openwrt-${arch}.${PACKAGE_FORMAT}"
